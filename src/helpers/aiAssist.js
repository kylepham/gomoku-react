export default function ai_move(board, boardDimension, ai, player) {
    ...
    function f_openTwo(currentP, x = null, y = null) {
      ...
      if (!x && !y) {
        ...
        let prop = temp[0],
          oppo = temp[1];
        if (currentP === player) [prop, oppo] = [oppo, prop];
        if (prop === 2 && board[x][y - 1] === 0 && board[x][y + 2] === 0) res++;
    ...
    function f_open_capped_Three(currentP, x = null, y = null) {
      ...
      if (!x && !y) {
        for (let i = 0; i < boardDimension; i++)
          for (let j = 0; j < boardDimension; j++) {
            ...
            if (
              ...
              board[i - 1] &&
              board[i + 3] &&
              board[i - 1][j + 1] + board[i + 3][j - 3] === -1 * currentP
            )
              resc3++;
            prop = temp[1][0];
            oppo = temp[1][1];
        ...
        let prop = temp[0],
          oppo = temp[1];
        if (currentP === player) [prop, oppo] = [oppo, prop];
        if (prop === 3 && board[x][y - 1] === 0 && board[x][y + 3] === 0) reso3++;
        if (prop === 3 && board[x][y - 1] + board[x][y + 3] === -1 * currentP)
          resc3++;
    ...
    function f_open_capped_Four(currentP, x = null, y = null) {
      ...
      if (!x && !y) {
        for (let i = 0; i < boardDimension; i++)
          for (let j = 0; j < boardDimension; j++) {
            ...
            if (
              ...
              board[i - 1] &&
              board[i + 4] &&
              board[i - 1][j + 1] + board[i + 4][j - 4] === -1 * currentP
            )
              resc4++;
            prop = temp[1][0];
            oppo = temp[1][1];
        ...
        let prop = temp[0],
          oppo = temp[1];
        if (currentP === player) [prop, oppo] = [oppo, prop];
        if (prop === 4 && board[x][y - 1] === 0 && board[x][y + 4] === 0) reso4++;
        if (prop === 4 && board[x][y - 1] + board[x][y + 4] === -1 * currentP)
          resc4++;
    ...
    function f_gappedFour_consecutiveFive(currentP, x = null, y = null) {
      ...
      if (!x && !y) {
        for (let i = 0; i < boardDimension; i++)
          for (let j = 0; j < boardDimension; j++) {
            ...
            if (
              ...
              board[i - 1][j + 1] + board[i + 5][j - 5] !== -2 * currentP
            )
              resg4++;
            if (
              prop === 5 &&
        ...
        if (
          ...
          resg4++;
        if (prop === 5 && board[x][y - 1] + board[x][y + 5] !== -2 * currentP)
          resc5++;
    ...
    function getGameState(aiStates, playerStates) {
      return (
        ...
        score["openThree"] * (aiStates["openThree"] - playerStates["openThree"]) +
        score["gappedThree"] *
          (aiStates["gappedThree"] - playerStates["gappedThree"]) +
        score["cappedThree"] *
          (aiStates["cappedThree"] - playerStates["cappedThree"]) +
        score["openFour"] * (aiStates["openFour"] - playerStates["openFour"]) +
        score["gappedFour"] *
    ...
    function getStatesOf(gamer, states, x, y) {
      ...
      if (x && y) {
        function validCoord(xx, yy) {
          return xx >= 0 && xx < boardDimension && yy >= 0 && yy < boardDimension;
        }
        ...
        if (gamer === board[x][y]) {
          ...
          for (let dir of directions) {
            ...
            if (!endsWithBlank1 && !endsWithBlank2) {
              ...
              else if (cnt1 + cnt2 + 1 === 3) statesClone["openThree"]++;
              else if (cnt1 + cnt2 + 1 === 4) statesClone["openFour"]++;
              else if (cnt1 + cnt2 + 1 === 5) statesClone["consecutiveFive"]++;
              ...
              if (endsWithBlank1) {
                ...
                else if (cnt2 === 5) statesClone["consecutiveFive"]--;
              } // blocked on side 1
              else {
                if (cnt1 === 3) statesClone["cappedThree"]--;
                else if (cnt1 === 4) statesClone["cappedFour"]--;
                else if (cnt1 === 5) statesClone["consecutiveFive"]--;
              }
  
              if (cnt1 + cnt2 + 1 === 3) statesClone["cappedThree"]++;
              else if (cnt1 + cnt2 + 1 === 4) statesClone["cappedFour"]++;
              else if (cnt1 + cnt2 + 1 === 5) statesClone["consecutiveFive"]++;
            }
          }
        } // blocking
          ...
          for (let dir of directions) {
            ...
            if (!endsWithBlank1 && !endsWithBlank2)
              ...
              if (cnt1 + cnt2 === 3 && cnt1 !== 0 && cnt2 !== 0)
                ...
                // if pattern is gapped four
                statesClone["gappedFour"]--;
              else if (cnt1 + cnt2 === 2 && (cnt1 === 0 || cnt2 === 0))
                statesClone["openTwo"]--;
  
              if (cnt1 === 3 || cnt2 === 3) statesClone["cappedThree"]++;
              else if (cnt1 === 4 || cnt2 === 4) statesClone["cappedFour"]++;
            } // one side is blocked, the other side is free
            else {
        ...
        if (gamer === ai) {
          ...
          let [o2_player, o3_c3_player, g3_player, o4_c4_player, g4_c5_player] = [
              f_openTwo(player),
    ...
    function negascout(
      ...
      if (
        ...
        return (isMaximizing ? 1 : -1) * getGameState(aiStates, playerStates);
    ...
    for (let i = 0; i < validMoves.length; i++) {
      ...
      let aiNewStates = getStatesOf(ai, ai_states, move.tuple[0], move.tuple[1]);
      let playerNewStates = getStatesOf(
        player,
        player_states,
        move.tuple[0],
